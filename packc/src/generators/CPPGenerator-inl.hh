...
struct CPPGenerator : public Generator{
...
// inherited from Generator
    ...
    virtual std::string generatePackDef(PackParser::PackDefContext *context) override{
        ...
        for(auto i : thisArgs){
            args.push_back(i.type + " " + i.name);
        }
        thisArgs = {};
        code += "pack::Pack " + context->ArguemnetName()->getText() + "(" + from(args).Join(", ") + ")";
        code += blockCode;
        return code;
    }
...
struct Clear{
    ...
    virtual antlrcpp::Any visitStart(PackParser::StartContext *context) override{
        ...
        this->codeHolder = R"(#include <libpack>)";
         codeHolder += "\n\n";
        ...
        if(!this->namespaceName.empty()){
            codeHolder += "};";
            codeHolder = from(codeHolder).replaceAll("{\n", "{\n\t")
                                         .replaceAll(";\n\t", ";\n\t\t")
                                         .replaceAll(";\n}", ";\n\t}")
                                         .replaceAll("\npack::", "\n\tpack::")
                                         .str();
